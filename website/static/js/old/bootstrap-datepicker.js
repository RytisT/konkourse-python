...
!function ( $ ) { 
  ...
  DatePicker.prototype = {
	...
	/*
	This part is different from the default. There's some discrepancy with using
	position:absolute after position:relative in CSS (one of its many bugs), so
	I had to include the offsetParent() method within the call chain.
	*/

/*        this.$picker.css({
          top: this.$el.outerHeight() + 2,
          left: (this.$el.offset().left - this.$el.offsetParent().offset().left)
        }).show();*/

        this.$picker.css({
          top: (this.$el.offset().top + this.$el.outerHeight() + 2 - this.$el.offsetParent().offset().top),
          left: (this.$el.offset().left - this.$el.offsetParent().offset().left)
        }).show();
